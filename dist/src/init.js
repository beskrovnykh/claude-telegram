...
function configTemplate(token, whitelist, permissionMode) {
    return `token: ${token}
workspace: .
whitelist:
...
function exampleModuleTemplate() {
...
export async function runInit(targetDir) {
    ...
    try {
        ...
        if (existsSync(configPath)) {
            const overwrite = await rl.question("claude-telegram.yaml already exists. Overwrite? (y/N) ");
        ...
        // Bot token
        const tokenAnswer = await rl.question("Bot token [${TELEGRAM_BOT_TOKEN}]: ");
        const token = tokenAnswer.trim() || "${TELEGRAM_BOT_TOKEN}";
        // Whitelist
        console.log("\nTip: run `npx claude-telegram whoami` to find your Telegram user ID.");
        const whitelistAnswer = await rl.question("Whitelist user IDs (comma-separated): ");
        ...
        if (invalidCount > 0) {
            console.log(`  Warning: ${invalidCount} invalid ID(s) ignored (must be positive integers).`);
        }
        if (whitelist.length === 0) {
            console.log("  ⚠ No user IDs entered.\n" +
